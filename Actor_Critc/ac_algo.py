...
for i in range(MAX_EPISODE):
    s = env.reset()
    ep_reward = 0
    total_delay = 0  # 用于记录每个 episode 的任务卸载时延
    j = 0
    while j < MAX_EP_STEPS:
        ...
        # 记录任务卸载时延
        total_delay += -r  # 因为 r 是负的 delay，所以这里取负号
        ...
        if j == MAX_EP_STEPS - 1 or is_terminal:
            print('Episode:', i, ' Steps: %2d' % j, ' Reward: %7.2f' % ep_reward, ' Delay: %7.2f' % total_delay)
            ep_reward_list = np.append(ep_reward_list, ep_reward)
            task_delay_list.append(total_delay)  # 记录卸载时延
            break
        j = j + 1

# 绘制 Episode vs Reward 图
plt.figure()
plt.plot(ep_reward_list)
plt.xlabel("Episode")
plt.ylabel("Reward")
plt.show()
with open("reward_ac.txt", "w") as file:
    for reward in ep_reward_list:
        file.write(f"{reward}\n")
# # 绘制 Episode vs 任务卸载时延 图
# plt.figure()
# plt.plot(task_delay_list)
# plt.xlabel("Episode")
# plt.ylabel("Delay (s)")
# plt.show()
