...
PRIORITY_EPSILON = 0.01  # 防止优先级为0
ALPHA = 0.6  # 控制优先级采样的程度
OUTPUT_GRAPH = False
...
class Memory:
    ...
    def sample(self, batch_size):
        ...
        probs = priorities / sum(priorities)  # 计算每个经验的采样概率
        indices = np.random.choice(len(self.memory), batch_size, p=probs)
        samples = [self.memory[i] for i in indices]
        return indices, samples

    def update_priorities(self, indices, td_errors):
        # 确保 td_errors 是可迭代的，并且与 indices 一一对应
        td_errors = np.array(td_errors).reshape(-1)  # 将其转换为一维数组
        for i, td_error in zip(indices, td_errors):
            priority = np.abs(td_error) + 1e-5  # 避免 priority 为 0
...
class DDPG:
    def __init__(self, a_dim, s_dim, a_bound):
        self.memory = Memory(MEMORY_CAPACITY)  # 使用优先级经验回放
        self.pointer = 0
        ...
        self.q_target = self.R + GAMMA * tf.minimum(self.q1_target, self.q2_target)

        # Critic 网络损失函数
        self.td_error1 = tf.losses.mean_squared_error(labels=self.q_target, predictions=self.q1_eval)
        self.td_error2 = tf.losses.mean_squared_error(labels=self.q_target, predictions=self.q2_eval)
        critic_loss = self.td_error1 + self.td_error2  # 最小化两个 Critic 网络的误差
        self.critic_train_op = tf.train.AdamOptimizer(LR_C).minimize(critic_loss, var_list=self.c1e_params + self.c2e_params)

        # Actor 网络损失函数：最大化 Critic Q1 值
        actor_loss = -tf.reduce_mean(self.q1_eval)
    ...
    def learn(self):
        ...
        # 从 Replay Buffer 中优先级采样
        indices, batch_memory = self.memory.sample(BATCH_SIZE)
        bs = np.array([mem[0] for mem in batch_memory])
        ba = np.array([mem[1] for mem in batch_memory])
        br = np.array([mem[2] for mem in batch_memory]).reshape(-1, 1)  # 修改为二维数组
        bs_ = np.array([mem[3] for mem in batch_memory])

        # 更新 Critic 网络
        td_error1, td_error2, _ = self.sess.run([self.td_error1, self.td_error2, self.critic_train_op],
                                                {self.S: bs, self.a: ba, self.R: br, self.S_: bs_})

        # 将返回的 td_error1 和 td_error2 确保转换为可迭代对象 (数组)
        td_error1 = np.array(td_error1).reshape(-1)  # 强制转换为一维数组
        td_error2 = np.array(td_error2).reshape(-1)

        # 更新优先级
        abs_errors = np.clip(np.abs(td_error1) + np.abs(td_error2), 0, 1.0)


        # 确保 abs_errors 是一个数组
        abs_errors = np.array(abs_errors).reshape(-1)
    ...
    def _build_actor(self, state, scope, trainable):
        with tf.variable_scope(scope):
            net = tf.layers.dense(state, 400, activation=lambda x: leaky_relu(x, alpha=0.2), trainable=trainable)
            net = tf.layers.dense(net, 100, activation=lambda x: leaky_relu(x, alpha=0.2), trainable=trainable)
            # 增加一层隐藏层
            net = tf.layers.dense(net, 20, activation=lambda x: leaky_relu(x, alpha=0.2), trainable=trainable)
            actions = tf.layers.dense(net, self.a_dim, activation=tf.nn.tanh, trainable=trainable)
            return tf.multiply(actions, self.a_bound[1], name='scaled_actions')
    ...
    def _build_critic(self, state, action, scope, trainable):
        with tf.variable_scope(scope):
            ...
            net = leaky_relu(tf.matmul(state, w1_s) + tf.matmul(action, w1_a) + b1, alpha=0.2)
            net = tf.layers.dense(net, 100, activation=lambda x: leaky_relu(x, alpha=0.2), trainable=trainable)
            # 增加一层隐藏层
            net = tf.layers.dense(net, 20, activation=lambda x: leaky_relu(x, alpha=0.2), trainable=trainable)
            return tf.layers.dense(net, 1, trainable=trainable)  # 输出 Q 值
