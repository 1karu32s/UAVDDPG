...
class UAVEnv(object):
    ...
    def step(self, action):  # 0: 选择服务的ue编号 ; 1: 方向theta; 2: 距离d; 3: offloading ratio
        ...
        # 飞行能耗
        e_fly = (dis_fly / self.t_fly) ** 2 * self.m_uav * self.t_fly * 0.5  # 飞行能耗计算
        # 服务器计算耗能
        t_server = offloading_ratio * task_size / (self.f_uav / self.s)  # 在UAV边缘服务器上计算时延
        e_server = self.r * self.f_uav ** 3 * t_server  # 计算能耗
        ...
        # 判断终止条件
        if self.sum_task_size == 0:  # 任务全部完成
            is_terminal = True
            reward = 0
        elif self.sum_task_size - self.task_list[ue_id] < 0:  # 最后一步任务和UE的任务不匹配
            self.task_list = np.ones(self.M) * self.sum_task_size
            reward = 0
            step_redo = True
        elif loc_uav_after_fly_x < 0 or loc_uav_after_fly_x > self.ground_width or loc_uav_after_fly_y < 0 or loc_uav_after_fly_y > self.ground_length:  # 超出边界
            reset_dist = True
            delay = self.com_delay(self.loc_ue_list[ue_id], self.loc_uav, offloading_ratio, task_size,
                                   block_flag)  # 计算延迟
            reward = -delay   # 更新奖励，包含能量惩罚
            self.e_battery_uav -= e_server  # 更新电池电量
